case $(uname) in
	...
	(Darwin)
		...
		if ! [[ -x /usr/local/bin/mvim ]] && [[ -x $VIM/../../MacOS/Vim ]]; then
			path=(/Applications/MacVim.app/Contents/MacOS(N))
		fi
		unset VIM
		path+=(~/.config/yarn/global/node_modules/.bin(N) ~/.yarn-config/global/node_modules/.bin(N) /usr/local/opt/gettext/bin(N) /usr/local/bin /usr/bin /bin /usr/sbin /sbin /usr/local/sbin(N) ~/bin(N))
		;;
esac
